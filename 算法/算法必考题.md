...
3sum 和为特定值的所有可能组合的下标 数据已经排好序

这道题和上一道题不一样的一点是　上一道题需要判断去重复　而这一道题需要把所有重复的组合下标都输出出来　不可以有遗漏

```java
public List<List<Integer>> getResult(int[]nums,int target)｛
   List<List<Integer>> result=new ArrayList<>();
　　HashMap<Integer> mapBegin=new HashMap<>（）；
　　HashMap<Integer> mapEnd=new HashMap<>();
   ｆｏｒ(int i=0;i<nums.length;i++){
       mapBegin.put(nums[i],i);
...
for(int i=0;i<nums.length-1;i++){
    ...
    while(begin<end){
        if(nums[begin]+nums[end]==target){
            for(int  w=mapBegin.get(nums[begin]);
                w<=mapEnd.get(nums[begin]);w++){
                for(int m=mapBegin.get(nums[end])；
                    ｍ<=mapEnd.get(nums[end]);m++){
            ...
            begin=mapEnd.get(nums[being])+1;
        }else{
            end=mapBegin.get(nums[end])-1;
        }
    }
}
   return result;
...
判断是否存在路径和为定值的路径（一条路径表示从根节点到叶子节点）
...
在二叉树中　无需从根节点开始　也无需到达叶子节点　只要是存在从父节点到子节点的和为定值　则是一条满足条件的路径　求总共有多少条这样的路径
...
public int pathSum(TreeNode root, int sum) {
    ...
    return getTotalNum(root,sum)+pathSum(root.left,sum)+pathSum(root.right,sum);
}
public int getTotalSum(TreeNode root,int sum){
    if(root==null){
        return 0;
    }
    return (root.val==sum?1:0)+getTotalSum(root.left,sum-root.val)+getTotalSum(root.right,sum-root.val);
...
先对角线翻转 然后如果是顺时针90度 则将第1列和第四列  第二列和第三列交换

逆时针90度则将第一行和第四行 第二行和第三行交换 
