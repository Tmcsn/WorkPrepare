...
1.7 在给定的０，１矩形中返回最大的矩形面积

这个题其实跟１．５题很类似　都可以采用同样的思路　一旦求的是二维矩形的面积　就可以将二维矩形压缩到一维求一维上的最大长度,这里同样适用　只需要将二维矩形不断的压缩　如果之前行和当前行都为１则压缩位置取１，否则取０，然后求连续的１的个数
...
public int maximalRectangle(char[][] matrix) {
    int max=0;
    ｆｏｒ(int i=1;i<matrix.length;i++){
        for(int j=0;j<matrix[0].length;j++){
            int totalTmp=0;
            matrix[i][j]=(matrix[i-1][j]=='1'&&matrix[i][j]==1)?'1':'0';
            if(matrix[i][j]=='1'){
                totalTmp+=1;
                max=Math.max(totalTmp*(i+1));  
            }
        }
    }
...
public int getDepth(TreeNode root){
    ...
    return Ｍａｔｈ.max(getDepth(root.left),getDepth(root.right))+1;
}
```
